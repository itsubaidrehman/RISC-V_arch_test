...
#constant for PMP_REG0_TOR,
.equiv PMP_REG2_TOR, 0x80004000 #8Kb region starting from 0x8000_2000 to 0x8000_4000 
...
supervisor_mode:
    ...
    # Apply PMP restrictions to Machine mode
    li t0, 0b10000         # Clear MPRV to enforce PMP in Machine mode
    csrs mstatus, t0
 
    li t0, 0x070F998C						# set the pmpcongif.L bit for TOR and NAPOT regions
    csrw pmpcfg0, t0
...
configure_pmp:
    ...
    srli t0, t0, 2;
    csrw pmpaddr0, t0;
	li t0, 0x0707070C; 						#grant execute permissions to TOR region
    csrw pmpcfg0, t0;
    ...
    csrw pmpaddr1, t0;
	li t0, 0x0707190C; 						#grant read permissions to NAPOT region 0x19 and execute permissions to TOR region 0x0C
    csrw pmpcfg0, t0;
    
	# TOR Region Setup 						# region for .text segment of the program
    li t0, PMP_REG2_TOR;					# TOR upto 0x80004000
    srli t0, t0, 2;
    csrw pmpaddr2, t0;
	li t0, 0x070F190C; 						#grant read, write and execute permissions to TOR region 2
    csrw pmpcfg0, t0;
...
switch_mode:
    ...
    and t2, t2, t3         # Clear existing MPP
    beq a0, zero, set_supervisor_mode
    li t3, 0x2 << 11       # Set MPP to User mode
    or t2, t2, t3
    j write_mstatus
set_supervisor_mode:
    li t3, 0x1 << 11       # Set MPP to Supervisor mode
    or t2, t2, t3
write_mstatus:
    csrw mstatus, t2
    ret
